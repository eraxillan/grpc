...
std::string GetStubHeaderPrologue(grpc_generator::File* file,
  ...
  std::string output;
  {
    ...
    printer->Print(vars, "#ifndef GRPC_stub_$filename_identifier$__INCLUDED\n");
    printer->Print(vars, "#define GRPC_stub_$filename_identifier$__INCLUDED\n");
    printer->Print(vars, "\n");
    printer->Print("#include <grpc++/grpc++.h>\n");
    printer->Print(vars, "#include \"$filename_base$$message_header_ext$\"\n");
    printer->Print(vars, file->additional_headers().c_str());
    printer->Print(vars, "\n");
  }
...
void PrintStubHeaderService(grpc_generator::Printer* printer,
  ...
  if (vars->find("Package") != vars->end()) {
    auto packageValue = (*vars)["Package"];
...
std::string GetStubHeaderEpilogue(grpc_generator::File* file,
  ...
  std::string output;
  {
    ...
    printer->Print(vars, "\n");
    printer->Print(vars, "#endif  // GRPC_stub_$filename_identifier$__INCLUDED\n");
...
std::string GetStubSourcePrologue(grpc_generator::File* file,
  ...
  std::string output;
  {
    ...
    printer->Print(vars,
                   "// If you make any local change, they will be lost.\n");
    printer->Print(vars, "// source: $filename$\n\n");
    printer->Print(vars, "#include \"utils.h\"\n");
    printer->Print(vars, "#include \"$filename_base$.stub.h\"\n");
    printer->Print(vars, "\n");
  }
  return output;
}
...
void PrintStubSourceServerMethod(grpc_generator::Printer* printer,
  ...
  (*vars)["Request"] = method->input_type_name();
  (*vars)["Response"] = method->output_type_name();
  (*vars)["Time"] = "current_time_string() + \" \"";
  (*vars)["CallID"] = "\"ID=\" + id + \" \"";
  (*vars)["Status"] = "grpc_error_code_string(status)";
  if (method->NoStreaming()) {
    printer->Print(*vars,
                   R"(
::grpc::Status $Service$ServiceImpl::$Method$(
  ...
  std::string json = message_as_json(*request);
  if (!json.empty()) {
    appendLineToLogFile($Time$ + $CallID$ + "REQUEST [$Package$$Service$/$Method$]:\n" + json);
  }
)");
    }
    printer->Print(*vars, 
  ...
  auto status = m_stub->$Method$(&clientContext, *request, response);)");
    if (method->name() != "HealthCheck") {
      printer->Print(
          *vars,
          R"(
  
  json = message_as_json(*response);
  if (!json.empty()) {
    appendLineToLogFile($Time$ + $CallID$ + "RESPONSE [$Package$$Service$/$Method$]:\n" + json);
    appendLineToLogFile($Time$ + $CallID$ + "ON_CLOSE [$Package$$Service$/$Method$]:\n" + $Status$);
  }
)");
    }
...
std::string GetServicesHeaderForwardDeclarations(grpc_generator::File* file,
  ...
  std::string output;
  {
    ...
    for (int i = 0; i < file->service_count(); ++i) {
      ...
      printer->Print(vars, "class $Service$ServiceImpl;\n");
    }
...
std::string GetServicesHeaderPointerDeclarations(grpc_generator::File* file,
  ...
  std::string output;
  {
    ...
    for (int i = 0; i < file->service_count(); ++i) {
      vars["Service"] = file->service(i)->name();
      printer->Print(vars,
                     "using $PackageRaw2$$Service$ServicePtr = "
                     "std::unique_ptr<$PackageRaw$::$Service$ServiceImpl>;\n");
    }
  }
  return output;
}
...
std::string GetServicesHeaderClassPrologue(grpc_generator::File* file,
  ...
  std::string output;
  {
    ...
    printer->Print("\n");
    printer->Print("class GrpcServices {\n");
  }
  return output;
}
...
std::string GetServicesHeaderClassEpilogue(grpc_generator::File* file,
  ...
  std::string output;
  {
    ...
    printer->Indent();
    printer->Print(
        "explicit GrpcServices(const std::shared_ptr<grpc::Channel> "
        "&channel);\n");
    printer->Print("~GrpcServices();\n");
    printer->Print("\n");
    printer->Print("void registerServices(grpc::ServerBuilder &builder);\n");
    printer->Outdent();
    printer->Print("};\n");
  }
  return output;
...
std::string GetServicesHeaderClassDeclaration(grpc_generator::File* file,
  ...
  std::string output;
  {
    ...
    for (int i = 0; i < file->service_count(); ++i) {
      vars["Service"] = file->service(i)->name();
      printer->Print(vars,
                     "$PackageRaw$$Service$ServicePtr "
                     "m_$PackageRaw$$Service$Service;\n");
    }
    printer->Outdent();
  }
  return output;
...
std::string GetServicesSourcePrologue(grpc_generator::File* file,
  ...
  std::string output;
  {
    ...
    printer->Print(vars,
                   "// If you make any local change, they will be lost.\n");
    printer->Print("#include \"services.h\"\n\n");
  }
  return output;
}
...
std::string GetServicesSourceConstructorPrologue(grpc_generator::File* file,
  ...
  std::string output;
  {
    ...
    printer->Print("\n");
    printer->Print(
        "GrpcServices::GrpcServices(const std::shared_ptr<grpc::Channel> "
        "&channel) {\n");
  }
  return output;
}
...
std::string GetServicesSourceConstructorDeclaration(grpc_generator::File* file,
  ...
  std::string output;
  {
    ...
    for (int i = 0; i < file->service_count(); ++i) {
      vars["Service"] = file->service(i)->name();
      printer->Print(vars,
                     "m_$PackageUnderscores$$Service$Service = "
                     "std::make_unique<$PackageColons$::$Service$"
                     "ServiceImpl>(channel);\n");
    }
    printer->Outdent();
  }
...
std::string GetServicesSourceDestructor(grpc_generator::File* file,
  ...
  std::string output;
  {
    ...
    printer->Print("GrpcServices::~GrpcServices() = default;\n");
  }
  return output;
}
...
std::string GetServicesSourceMethodPrologue(grpc_generator::File* file,
  ...
  std::string output;
  {
    ...
    printer->Print("\n");
    printer->Print("void GrpcServices::registerServices(grpc::ServerBuilder &builder) {\n");
  }
  return output;
}
...
std::string GetServicesSourceMethodCall(grpc_generator::File* file,
  ...
  std::string output;
  {
    ...
    for (int i = 0; i < file->service_count(); ++i) {
      vars["Service"] = file->service(i)->name();
      printer->Print(vars,
                     "builder.RegisterService(m_$PackageUnderscores$$Service$"
                     "Service.get());\n");
    }
    printer->Outdent();
  }
...
std::string GetUtilsHeaderPrologue(grpc_generator::File* file,
  ...
  std::string output;
  {
    ...
    vars["filename_identifier"] = FilenameIdentifier("utils.h");

    printer->Print(vars, R"(// Generated by the gRPC C++ plugin.
// If you make any local change, they will be lost.
...
inline std::string current_time_string() {
  ...
  const auto t_c = std::chrono::system_clock::to_time_t(now_time);

  auto ms = std::chrono::duration_cast<std::chrono::milliseconds>(now_time.time_since_epoch()) -
            std::chrono::duration_cast<std::chrono::seconds>(now_time.time_since_epoch());

  std::stringstream ss;
  ss << std::put_time(std::localtime(&t_c), "%F %T.") << ms.count() << std::flush;
  return ss.str();
}
    ...
    printer->Print(R"(
inline std::string grpc_error_code_string(grpc::Status status) {
...
namespace uuid {
  ...
  inline std::string generate_uuid_v4() {
    ...
    printer->Print("void appendLineToLogFile(const std::string& text);\n");
  }
  return output;
}
    ...
    for (int i = 0; i < file->service_count(); ++i) {
      ...
      for (int j = 0; j < service->method_count(); ++j) {
        ...
        if (!leadingComments.empty()) {
          printer->Print(("<!-- " + grpc_cpp_generator::trim_copy(leadingComments) + " -->\n").c_str());
        }
        ...
        if (!trailingComments.empty()) {
          printer->Print(("<!--" + grpc_cpp_generator::trim_copy(trailingComments) + "-->\n").c_str());
        }
      }
